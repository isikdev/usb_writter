...
import os
from creedsolo.creedsolo import RSCodec 
...
class PartitionHandler:
    ...
    def __init__(self, partition_letter: str, recovery_percent: int = 10, buffer_size: int = 128 * 1024):
        ...
        if buffer_size % 512 != 0:
            log.warning("Buffer size is not divisible by 512, rounding up to the next multiple of 512")
        ...
        self.parity_size = min(int(self.MAX_BLOCK_SIZE * recovery_percent / 100), self.MAX_BLOCK_SIZE - 1)
    ...
    def create_parity(self, output_file: str) -> None:
        log.info(f"Creating {self.recovery_percent}% parity data for partition {self.disk_handler.disk_letter}")
        ...
        total_buffers = (self.partition_size + self.buffer_size - 1) // self.buffer_size
        ...
        with open(output_file, 'wb') as f:
            ...
            with tqdm(total=total_buffers, desc="Processing buffers", unit="buffer") as pbar:
                for buffer_idx in range(total_buffers):
                    ...
                    chunks = [buffer_data[i:i + self.data_size] for i in range(0, len(buffer_data), self.data_size)]
                    ...
                    for chunk in chunks:
                        ...
                        if len(chunk_array) < self.data_size:
                            chunk_array.extend([0] * (self.data_size - len(chunk_array)))
                        encoded = self.rs.encode(chunk_array)
                        parity = encoded[len(chunk_array):]  # Get only parity bytes
                        all_parity.extend(parity)
                    
                    # Save buffer index and parity
                    f.write(buffer_idx.to_bytes(8, 'little'))
                    f.write(len(all_parity).to_bytes(8, 'little'))  # Store parity size
                    f.write(all_parity)
    ...
    def verify_and_repair(self, parity_file: str) -> Tuple[int, int]:
        ...
        with open(parity_file, 'rb') as f:
            ...
            if stored_recovery != self.recovery_percent:
                raise ValueError(Msg.Err.parity_created_with_dif_recovery_percent(self.recovery_percent, stored_recovery))
            if stored_buffer_size != self.buffer_size:
                raise ValueError(Msg.Err.parity_created_with_dif_buffer_size(self.buffer_size, stored_buffer_size))
            ...
            with tqdm(total=total_buffers, desc="Verifying data", unit="buffer") as pbar:
                while True:
                    ...
                    for chunk_idx, chunk in enumerate(chunks):
                        ...
                        stored_chunk_parity = all_parity[chunk_idx * self.parity_size:(chunk_idx + 1) * self.parity_size]
                    ...
                    if needs_repair:
                        ...
                        parity_chunks = [all_parity[i:i + self.parity_size] for i in range(0, len(all_parity), self.parity_size)]
                        ...
                        for chunk_idx, (chunk, parity) in enumerate(zip(chunks, parity_chunks)):
                            ...
                            try:
                                ...
                                decoded, _, errata_pos = self.rs.decode(full_data)
                                errors_list = [pos for pos in errata_pos if pos < len(chunk)]
                                
                                if errors_list:  # If errors were found in actual data
                                    errors_found += len(errors_list)
                                    buffer_corrected = True
                                    log.warning(f"Found {len(errors_list)} errors in chunk {chunk_idx} of buffer {buffer_idx}")
                                
                                # Add decoded chunk to corrected buffer
                                corrected_buffer.extend(decoded[:len(chunk)])
                                
                            except rs.ReedSolomonError as e:
                                log.error(f"Unable to correct errors in chunk {chunk_idx} of buffer {buffer_idx}: {str(e)}")
                                errors_found += 1
                                corrected_buffer.extend(chunk)

                        # Write back corrected buffer if needed
                        if buffer_corrected:
                            self._write_buffer(buffer_offset, bytes(corrected_buffer))
                            errors_corrected += 1
                            log.warning(f"Corrected errors in buffer {buffer_idx}")
